...
module.exports = {
  start: async function (api) {
    ...
    Logger.info('start waitingValidators');
    const waitingValidators = await api.derive.staking.waitingInfo();
    // Logger.debug(waitingValidators);
    const stakingInfo = await module.exports.getStakingInfo(api, waitingValidators.waiting);
    // Logger.debug(stakingInfo);
    const stakingInfoWithRewards = await module.exports.getEstimatedPoolReward(
      api,
    ...
    const WaitingValidators = Container.get('WaitingValidators') as mongoose.Model<IStakingInfo & mongoose.Document>;
    try {
      await WaitingValidators.deleteMany({});
      await WaitingValidators.insertMany(stakingInfoWithRewards);
    } catch (error) {
      Logger.error(error);
    }
    Logger.info('stop waitingValidators');
  },

  getStakingInfo: async function (api, waitingValidators): Promise<Array<IStakingInfo>> {
    await wait(5000);
    const stakingInfo = await Promise.all(waitingValidators.map((valId) => api.derive.staking.account(valId)));
  ...
  getEstimatedPoolReward: async function (api, waitingValidators, stakingInfo: Array<IStakingInfo>) {
    await wait(5000);
    // const Logger = Container.get('logger');
    const waitingValidatorsArr = waitingValidators.map((x) => x.toString());
    ...
    const historyData = await ValidatorHistory.aggregate([
      {
        $match: { stashId: { $in: waitingValidatorsArr } },
      },
      {
        $group: {
          _id: '$stashId',
